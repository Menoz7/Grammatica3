 ...
 Value *VariableExprAST::codegen(driver& drv) {
  AllocaInst *variabile = drv.NamedValues[Name];
  
  if (!variabile){
    //Se la variabile non viene trovata, la si cerca tra le globali
    GlobalVariable* Global = module->getGlobalVariable(Name);

    if(!Global){
      return LogErrorV("Variabile "+Name+" non trovata");
    }

    else{
      //genero la load, se trovo la variabile tra le globali
      return builder->CreateLoad(Type::getDoubleTy(*context), Global, Name.c_str());
    }

  }

  return builder->CreateLoad(variabile->getAllocatedType(), variabile, Name.c_str());
}
  ...
  Value *BinaryExprAST::codegen(driver& drv) {
    if (!LHS){
      //Nel caso in cui non sia definita un'operazione binaria. 
      if(Op=='-')
      {
          return builder->CreateFNeg(RHS->codegen(drv), "neg"); //negativo
      }
      else 
          return LogErrorV("Operazione non definita correttamente ");
    }
  ...
  Value* IfExprAST::codegen(driver& drv) {
      ...
      builder->SetInsertPoint(TrueBB); //posizionamento del BB
      Value *TrueV = TrueExp->codegen(drv); //creazione del codice
      if (!TrueV)
        return nullptr;
      builder->CreateBr(MergeBB); //crea salto condizionato al blocco merge
...
Function *FunctionAST::codegen(driver& drv) {
  ...
  // Ora può essere generato il codice corrispondente al body (che potrà
...
/***************************************************************
 * *************************************************************/
/*      AGGIUNTE---FUNZIONI SCRITTE DA ME                      */
...
//crea un oggetto di tipo Type che rappresenta il tipo double (ritornato dalla fuinzione getDoubleTy())
Value* VarGlobalAST::codegen(driver& drv){ Type* doubleType = Type::getDoubleTy(*context);    
  //viene creata una variabile globale di tipo double 
  ...
  globalVar->print(errs()); //stampa la dichiarazione della variabile
  fprintf(stderr, "\n"); //crea una nuova riga
  return nullptr; //chiude la funzione
};
  

/****************************STMTSAST*******************************/

/// StmtsAST funzioni Costruttore e ridefinizione di codgen
StmtAST::StmtAST(ExprAST* Expression, ExprAST* Statement): Expression(Expression), Statement(Statement) {}; 
//il costruttore prende in ingresso due oggetti ExprsAST e li assegna alle variabili Expression Statement

//metodo responsabile della generazione di codice llvm per l'insieme di istruzioni rappresentato da StmtsAST
Value* StmtAST::codegen(driver& drv) {
  Value* begin=Expression->codegen(drv); //chiama codgen su Expression e assegna il valore alla variabile begin
  
  if (!begin){
    return nullptr;
  }
  if (Statement){ //controlla che siano state fornite delle istruzioni, se Statement è diverso da nullptr
    Value* continuation = Statement->codegen(drv); //se si entra nell'if si chiama il codegen sull'espressione e assegna il valore restituito a continuation
    if(!continuation){ //se è nullo vuol dire che la generazione del codice Statement ha generato degli errori  o non ha generato alcun valore
      return nullptr;
    }
    return continuation; //fondamentale per assegnare il giusto tipo alla variabile
  }
  return begin; //Se tutto è andatao bene restituisce il valore begin che rappresenta il valore risultante dell'espressione generata dall'insieme di istruzioni.
};
...
///AssignmentAST classe per l'assegnazione di un valore ad una variabile all'interno del programma
//L'inizializzazione di Val a nullptr nel costruttore permette di avere un valore predefinito sicuro per Val.
AssignmentAST::AssignmentAST(std::string Name, ExprAST* Val = nullptr): Name(Name), Val(Val) {};
//Gestione operatore '++'. 
AssignmentAST::AssignmentAST(std::string Name, char op): Name(Name), Op(op) {};

const std::string& AssignmentAST::getName() const { //nome associato all'oggetto assignment
   return Name; 
};
...
Value* AssignmentAST::codegen(driver& drv) {
  // Viene subito recuperato il riferimento alla funzione in cui si trova
  // il blocco corrente. Il riferimento è necessario perché lo spazio necessario    
  // per memorizzare una variabile viene sempre riservato nell'entry block della funzione.
  // l'allocazione viene fatta tramite l'utility CreateEntryBlockAlloca
  if (Op == '+'){
    Function* f = builder->GetInsertBlock()->getParent(); //recupera il riferimento alla funzione in cui il blocco è inserito

    //La variabile deve essere definita 
    Value* val = drv.NamedValues[Name];
    if (!val) return LogErrorV("Variabile non definita");
    
    //Viene generata una nuova istruzione
    Value* newinst = builder->CreateLoad(Type::getDoubleTy(*context), val, Name);

    //Creo la somma
    Value* sum = builder->CreateFAdd(newinst, ConstantFP::get(*context, APFloat(1.0)), "inc");//incremento
    //la salvo con una store
    Value* store = builder->CreateStore(sum, val);
      
    return val;
  }
  //in tutti i restanti casi
  Value *varval = Val->codegen(drv);
  if (!varval)
    return nullptr;
  ...
  if (!val){
    //Controllo se la variabile è globale
    GlobalVariable* Global = module->getGlobalVariable(Name);

    if(!Global) return LogErrorV("Variabile non definita!");  

    builder->CreateStore(varval, Global);
    return Global;
    }
  builder->CreateStore(varval, val);
  return val;
};

  /************************* If Expression Tree *************************/

  //If già implementato dal prof ma viene riiscritto 
  IffExprAST::IffExprAST(ExprAST* Cond, ExprAST* TrueExp, ExprAST* FalseExp):
    Cond(Cond), TrueExp(TrueExp), FalseExp(FalseExp) {};
    
  Value* IffExprAST::codegen(driver& drv) {
      // Viene  generato il codice per valutare la condizione, che
      // memorizza il risultato (di tipo i1, dunque booleano) nel registro SSA 
      // che viene "memorizzato" in CondV. 
      Value* CondV = Cond->codegen(drv);
      if (!CondV)
      ...
      BasicBlock *MergeBB = BasicBlock::Create(*context, "endcond");
      // Gli altri due blocchi non vengono ancora inseriti perché le istruzioni
      // previste nel "ramo" true possono generare altri blocchi, che naturalmente andrebbero inseriti prima di FalseBB
      ...
      // Ne consegue che il blocco corrente potrebbe non coincidere più con TrueBB.
      // Il branch alla parte merge deve però essere effettuato dal blocco corrente,
      // che dunque va recuperato. Ed è fondamentale sapere da quale blocco si origina
      // il salto perché tale informazione verrà utilizzata da un'istruzione PHI.
      // Nel caso in cui non sia stato inserito alcun nuovo blocco, la seguente
      // istruzione corrisponde ad una NO-OP
      TrueBB = builder->GetInsertBlock();
      ...
      //Si genera la parte dove i due flussi si uniscono -> si deve posizionare correttamente il builder, in modo che punti al giusto BB
      builder->SetInsertPoint(MergeBB);
...
ForExprAST::ForExprAST(std::variant<VarBindingAST*, AssignmentAST*> first, ExprAST* cond, AssignmentAST* step, ExprAST* body):
   First(first), Cond(cond), Step(step), Body(body) {};
   
Value* ForExprAST::codegen(driver& drv) {
  // First può essere o un VarBinding o un Assignment, dipende se la variabile è già stata definita o meno.
  std::string namevar;
  if (std::holds_alternative<VarBindingAST*>(First)){
    namevar = std::get<VarBindingAST*>(First)->getName();}
  else {if (std::holds_alternative<AssignmentAST*>(First)){
    namevar = std::get<AssignmentAST*>(First)->getName();}
  else return nullptr;}

  AllocaInst* tmp;
  //Se viene trovata la variabile, viene memorizzata
  if (drv.NamedValues[namevar] != nullptr){
    tmp = drv.NamedValues[namevar];
  }
  else {//Viceversa, viene generata
    if (std::holds_alternative<VarBindingAST*>(First)){
      drv.NamedValues[namevar] = std::get<VarBindingAST*>(First)->codegen(drv);}
    else{ if (std::holds_alternative<AssignmentAST*>(First)){
      drv.NamedValues[namevar] = std::get<VarBindingAST*>(First)->codegen(drv);}}
  }

  Function* fun = builder->GetInsertBlock()->getParent();
  AllocaInst *Alloca = CreateEntryBlockAlloca(fun, namevar);

  //Simili per struttura al costrutto if
  ...
  //Blocco condizione
  Value* CondV = Cond->codegen(drv);//genero il codice
  //CondV è un puntatore a Value che rappresenta il risultato della generazione del codice ->condizione del loop
    if (!CondV) return nullptr;
  CondBB = builder->GetInsertBlock();//restituisce il blocco corrente in cui il builder sta inserendo le istruzioni
  //CondBB memorizza il blocco
  fun->insert(fun->end(), LoopBB); //inserisco il blocco LoopBB alla fine della funzione assicurandosi che il loop ne faccia parte.
  
  //Codice per il salto condizionato
  //se è true entra nel blocco LoopBB in caso false esce dal loop e va nel blocco MergeBB
  builder->CreateCondBr(CondV, LoopBB, MergeBB);

  //Creazione blocco Loop
  ...
  //aggiornamento come blocco corrente del BB
  LoopBB = builder->GetInsertBlock();
  //Alla fine del corpo del LoopBB, viene generato un salto che riporta alla condizione CondBB
  builder->CreateBr(CondBB);
  fun->insert(fun->end(), MergeBB);
  //Inserimento del blocco Merge
  builder->SetInsertPoint(MergeBB);
  //Se a suo tempo è stato creato e assegnato un valore tmp, 
  //viene messo il valore dentro la variabile VarName, così che possa essere aggiornato nel caso la variabile non sia stata definita.
  if (tmp){
    drv.NamedValues[namevar] = tmp;
  }
  return fun;
};  
...
/********************CONDEXPRAST***************************/
CondExprAST::CondExprAST(char Op, ExprAST* lside, ExprAST* rside): Op(Op), lside(lside), rside(rside) {};

Value *CondExprAST::codegen(driver& drv) {

  //Caso del NOT, che non ha la parte sinistra 
  if (!lside && Op == '!')
    return builder->CreateNot(rside->codegen(drv),"not");

  Value *R = rside->codegen(drv);
  Value *L = lside->codegen(drv);
  if (!L || !R) 
     return nullptr;
  switch (Op) {
  case '|':
  ...
  default:  
    std::cout << Op << std::endl;
    return LogErrorV("Confronto non definito!");
  }
};