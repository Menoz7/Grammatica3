...
#define DRIVER_HPP
/************************* IR related modules ******************************/
#include "/usr/lib/llvm-17/include/llvm/ADT/APFloat.h"
...
///VarGlobalAST- Classe che istanzia una variabile globale
...
///StmstsAST classe per lo statement(insieme di istruzioni)
...
///AssignmentAST classe per l'assegnamento 
...
/*
il blocco IfExprAST è già stato implementato dal prof
*/
/// IfExprAST
class IffExprAST : public ExprAST {
private:
  ExprAST* Cond;
  ExprAST* TrueExp;
  ExprAST* FalseExp;
public:
  IffExprAST(ExprAST* Cond, ExprAST* TrueExp, ExprAST* FalseExp);
  Value *codegen(driver& drv) override;
};
...
private:
  ExprAST* Body;
  std::variant<VarBindingAST*, AssignmentAST*> First;
  AssignmentAST* Step;
  ExprAST* Cond;
public:
  ForExprAST(std::variant<VarBindingAST*, AssignmentAST*> First, ExprAST* Cond, AssignmentAST* Step, ExprAST* Body);
  Value *codegen(driver& drv) override;
};
...
public:
  CondExprAST(char Op, ExprAST* lside, ExprAST* rside);
  Value *codegen(driver& drv) override;
};
